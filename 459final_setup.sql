CREATE DATABASE IF NOT EXISTS `sport_tracker`;
USE `sport_tracker`;

CREATE TABLE IF NOT EXISTS `coaches` (
  `Coach_ID` smallint(5) UNSIGNED NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `games` (
  ...
  PRIMARY KEY (`Game_ID`),
  KEY `H_Team_ID` (`H_Team_ID`),
  KEY `A_Team_ID` (`A_Team_ID`),
  CONSTRAINT `games_ibfk_1` FOREIGN KEY (`H_Team_ID`) REFERENCES `teams` (`Team_ID`) ON DELETE CASCADE,
  CONSTRAINT `games_ibfk_2` FOREIGN KEY (`A_Team_ID`) REFERENCES `teams` (`Team_ID`) ON DELETE CASCADE
);

INSERT INTO `games` (`Game_ID`, `H_Team_ID`, `A_Team_ID`, `H_Team_Score`, `A_Team_Score`, `Game_Date`) VALUES
(1, 1, 2, 112, 108, '2024-01-15'),
(2, 3, 4, 98, 105, '2024-01-16'),
(3, 5, 6, 120, 110, '2024-01-17'),
(4, 7, 8, 115, 117, '2024-01-18'),
(5, 9, 10, 104, 95, '2024-01-19'),
(6, 11, 12, 102, 100, '2024-01-20'),
(7, 13, 14, 110, 109, '2024-01-21'),
(8, 15, 16, 121, 113, '2024-01-22'),
(9, 17, 18, 106, 107, '2024-01-23'),
(10, 19, 20, 95, 99, '2024-01-24');
...
CREATE TABLE IF NOT EXISTS `team_stats` (
  `Team_Stat_ID` smallint(5) UNSIGNED NOT NULL AUTO_INCREMENT,
  `Team_ID` smallint(5) UNSIGNED NOT NULL,
  `Wins` smallint(5) UNSIGNED NOT NULL,
  `Losses` smallint(5) UNSIGNED NOT NULL,
  PRIMARY KEY (`Team_Stat_ID`),
  KEY `Team_ID` (`Team_ID`),
  CONSTRAINT `team_stats_ibfk_1` FOREIGN KEY (`Team_ID`) REFERENCES `teams` (`Team_ID`) ON DELETE CASCADE
);
...
SELECT Player_ID, 0
FROM players
WHERE Player_ID NOT IN (SELECT Player_ID FROM player_stats);
